      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants.length
          ? `<ul class="participant-list">${details.participants
              .map(
                (participant) =>
                  `<li class="participant-row"><span>${participant}</span><button type="button" class="remove-participant" data-email="${participant}" aria-label="Remove ${participant}" title="Remove participant">&#128465;</button></li>`
              )
              .join("")}</ul>`
